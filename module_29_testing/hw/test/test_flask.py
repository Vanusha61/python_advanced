...
def test_create_good_client_201(client):
    ...
    result = {'client': {'car_number': '123123123', 'credit_card': '123123123', 'id': 2, 'name': 'name1',
                         'surname': 'surname1'}}
...
def test_create_good_parking_201(client):
    ...
    result = {
        'parking': {'address': '123123123', 'count_available_places': 3, 'count_places': 3, 'id': 2, 'opened': True}}
    assert response.status_code == 201
    assert response.get_json() == result
...
def test_create_good_client_parking_201(client):
    ...
    result = {'client_parking': {'client_id': 2, 'id': 2, 'parking_id': 2,
                                 'time_in': f"{response.get_json()['client_parking']['time_in']}", 'time_out': None}}
    assert response.status_code == 201
    assert response.get_json() == result
