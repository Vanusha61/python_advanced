...
def create_app():
    ...
    def created_parkings():
        ...
        if not address or not count_places or not count_available_places:
            return (
                jsonify(
                    {
                        "error": "Please provide both address,"
                        " and count_place and count_available_places"
                    }
                ),
                400,
            )
    ...
    def created_client_parking():
        ...
        if not client_id or not parking_id:
            return (
                jsonify({"error": "Please provide both client_id and parking_id"}),
                400,
            )
            ...
            jsonify(
                {
                    "client_parking": {
                        ...
                        "time_in": (
                            client_parking.time_in.isoformat()
                            if client_parking.time_in
                            else None
                        ),
                        "time_out": (
                            client_parking.time_out.isoformat()
